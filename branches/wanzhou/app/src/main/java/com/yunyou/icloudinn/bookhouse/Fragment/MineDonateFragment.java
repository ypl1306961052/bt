...
public class MineDonateFragment extends Fragment {
    ...
    private void initMyview(View view) {
        ...
        mCommonAdapter=new CommonAdapter<DonateBookData.DataBeanX.DataBean>(getActivity(),R.layout.list_item_donate_book,mListData) {
            @Override
            protected void convert(ViewHolder holder, DonateBookData.DataBeanX.DataBean dataBean, int position) {
...
//                holder.setText(R.id.item_rent_number_borrow,dataBean.getBook().getModel().getName());
    ...
    private void initData() {
        ...
        customProgress.show();
        OkHttpUtils.get()
                .addParams("user_id",user.getData().getYunsu_id()+"")
                .url(Api.DONATE_BOOK)
                .build()
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        try {
                            ...
                            if (json.getInteger("code") == 100) {
                                ...
                                if ((JSON.parseObject(response,DonateBookData.class)).getData().getData().size()==0){
